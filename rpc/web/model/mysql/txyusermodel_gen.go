...
func (m *defaultTxyUserModel) Insert(ctx context.Context, data *TxyUser) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Uid, data.Type, data.Nickname, data.HeadImg, data.Openid, data.AccessToken, data.Ctime, data.Mtime, data.LastLoginTime, data.LastLoginIp, data.LoginTimes, data.Status, data.Email, data.Password, data.IsAdmin, data.Gold, data.Score, data.Conscore)
	return ret, err
}

func (m *defaultTxyUserModel) Update(ctx context.Context, data *TxyUser) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, txyUserRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, data.Uid, data.Type, data.Nickname, data.HeadImg, data.Openid, data.AccessToken, data.Ctime, data.Mtime, data.LastLoginTime, data.LastLoginIp, data.LoginTimes, data.Status, data.Email, data.Password, data.IsAdmin, data.Gold, data.Score, data.Conscore, data.Id)
	return err
}
