...
func (m *defaultTxyUserModel) Update(ctx context.Context, data *TxyUser) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.Uid, data.Type, data.Nickname, data.HeadImg, data.Openid, data.AccessToken, data.Ctime, data.Mtime, data.LastLoginTime, data.LastLoginIp, data.LoginTimes, data.Status, data.Email, data.Username, data.Password, data.IsAdmin, data.Gold, data.Score, data.Conscore, data.Id)
	return err
}
